...
char* create_response_header(int sock_index, uint8_t control_code, uint8_t response_code, uint16_t payload_len)
{
    char *buffer;
        ...
        BUILD_BUG_ON(sizeof(struct CONTROL_RESPONSE_HEADER) != CONTROL_RESPONSE_HEADER_SIZE); // This will FAIL during compilation itself; See comment above.
    ...
    buffer = (char *) malloc(sizeof(char)*CONTROL_RESPONSE_HEADER_SIZE);
    #ifdef PACKET_USING_STRUCT
